...
const VALID_PERIODS: FinancePeriod[] = [
    "today",
    "month",
    "3months",
    "6months",
...
export default async function FinanzasPage({ searchParams }: Props) {
    ...
    const period: FinancePeriod = VALID_PERIODS.includes(
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title="Finanzas"
                description="Ingresos y sesiones por psicólogo"
                actions={
                    <>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div className="rounded-lg border border-border bg-card p-4">
                    ...
                    <p className="mt-1 text-2xl font-bold">
                        {formatUSD.format(summary.totalRevenueUsd)}
                    </p>
                    {summary.totalRevenueByCurrency.length > 0 && (
                        <p className="mt-1 text-xs text-muted-foreground">
                            {formatCurrencyBreakdown(
                                summary.totalRevenueByCurrency,
                            )}
                        </p>
                    )}
                </div>
                ...
                <div className="rounded-lg border border-border bg-card p-4">
                    ...
                    <p className="mt-1 text-2xl font-bold">
                        {formatUSD.format(summary.netRevenueUsd)}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                        Recaudado menos comisiones a psicólogos y comisión de
            ...
            <p className="text-xs text-muted-foreground">
                ...
                diario. El desglose por moneda es el monto real cobrado.
            </p>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        {p.totalRevenueByCurrency.length >
                                            0 && (
                                            <p className="text-xs text-muted-foreground">
                                                {formatCurrencyBreakdown(
                                                    p.totalRevenueByCurrency,
                                                )}
                                            </p>
                                        )}
